...
if(isset($_POST['submit'])){
	...
	if(strlen($username)<3){
		...
		$stmt = $db->prepare('SELECT username FROM docmem WHERE username = :username');
	...
	if(!filter_var($email, FILTER_VALIDATE_EMAIL)){
		...
		$stmt = $db->prepare('SELECT email FROM docmem WHERE email = :email');
	...
	if(!isset($error)){
		...
		try {
			...
			$stmt = $db->prepare('INSERT INTO docmem (username,password,email) VALUES (:username, :password, :email)');
			$stmt->execute(array(
				':username' => $username,
				':password' => $password,
				':email' => $email
			...
			header('Location: docreg.php?action=joined');
			exit;
	...
	<div class="row">
	    ...
	    <div class="col-xs-12 col-sm-8 col-md-6 col-sm-offset-2 col-md-offset-3">
			<form role="form" method="post" action="" autocomplete="off">
				<h2>Admin -You can add a doctor 				<a href="adminop.php">
<img src="bak.jpg" alt="" width="42" height="40" border=""></a></h2>
				...
				if(isset($_GET['action']) && $_GET['action'] == 'joined'){
					echo "<h2 class='bg-success'>You have successfully added a Doctor to your organisation.</h2>";
				}
				?>
